...
//				((int) width) + getPaddingLeft(), currentTab.getTop() + getPaddingTop() + category_text.getTop() + category_text.getHeight());
		/**
         * getHeight()  得到水平滚动条的高度      距离top和距离bottom改为 2dp就ok了
		 */
		rect.set(((int) left) + getPaddingLeft(), 99,
				((int) width) + getPaddingLeft(), 105);
//		Log.d("dasdasdsd",getPaddingTop() +"   "+category_text.getHeight()+"   "+category_text.getBottom()+"   "+getHeight());
	}
